 ...
 *
 *   另外提供{@link #startImageCycle() } \ {@link #pauseImageCycle() }两种方法，用于在Activity不可见之时节省资源；
...
public class ImageCycleView extends FrameLayout {
     ...
     * 小圆点间隔的一半(px)
     */
    private final int POINT_PADDING = 6;
    ...
    public void setImageResources(ArrayList<ADInfo> infoList, ImageCycleViewListener imageCycleViewListener) {
        ...
        if (imageCount > 1) {
            ...
            for (int i = 0; i < imageCount; i++) {
                ...
                mImageView.setPadding(POINT_PADDING, 0, POINT_PADDING, 0);
...
//				}
     ...
     * @param index
     * @author 王鑫磊
     * @date 2016-6-6  下午1:22:52
     * @class ImageCycleView.java
     * @description 滑动到边缘
     */
    private void turnToEnd(final int index) {
        mHandler.postDelayed(new Runnable() {

            @Override
            public void run() {
                if (index == 0) {
                    mBannerPager.setCurrentItem(mImageViews.length, false);
                }
    ...
    private class ImageCycleAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            String imageUrl = mAdList.get(position).getImageUrl();
